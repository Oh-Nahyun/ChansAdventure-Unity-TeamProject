...
public class ObjectPool<T> : MonoBehaviour where T : RecycleObject // where(조건) T : RecycleObject
                                                                   // T타입은 RecycleObject이거나 RecycleObject를 상속받은 클래스만 가능하다.
{
    /// <summary>
    /// 풀에서 관리할 오브젝트의 프리팹
    /// </summary>
    ...
    public void Initialize()
    {
        ...
        {
            foreach (T obj in pool) // foreach : 특정 컬랙션 안에 있는 모든 요소를 한 번씩 처리해야 할 일이 있을 때 사용
            {
    ...
    void GenerateObjects(int start, int end, T[] results)
    {
        for (int i = start; i < end; i++)
        {
            ...
            //readyQueue.Enqueue(comp); // 레디큐에 추가하고 (위의 델리게이트 등록한 것 때문에 아래에서 비활성화 하면 자동으로 처리)
